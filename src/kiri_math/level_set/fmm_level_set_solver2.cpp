    ...
    ScalarGrid2 *outputSdf)
{
    ...
    markers.forEachIndex([&](size_t i, size_t j) {
        if (!isInsideSdf(output(i, j)) && ((i > 0 && isInsideSdf(output(i - 1, j))) || (i + 1 < size.x && isInsideSdf(output(i + 1, j))) || (j > 0 && isInsideSdf(output(i, j - 1))) || (j + 1 < size.y && isInsideSdf(output(i, j + 1)))))
        {
            output(i, j) = solveQuadNearBoundary(
                markers, output, gridSpacing, invGridSpacingSqr, 1.0, i, j);
        }
        else if (isInsideSdf(output(i, j)) && ((i > 0 && !isInsideSdf(output(i - 1, j))) || (i + 1 < size.x && !isInsideSdf(output(i + 1, j))) || (j > 0 && !isInsideSdf(output(i, j - 1))) || (j + 1 < size.y && !isInsideSdf(output(i, j + 1)))))
        {
            output(i, j) = solveQuadNearBoundary(
                markers, output, gridSpacing, invGridSpacingSqr, -1.0, i, j);
        }
    ...
    for (int sign = 0; sign < 2; ++sign)
    {
        ...
        std::priority_queue<
            Point2UI, std::vector<Point2UI>, decltype(compare)>
            trial(compare);
        markers.forEachIndex([&](size_t i, size_t j) {
            if (markers(i, j) != kKnown && ((i > 0 && markers(i - 1, j) == kKnown) || (i + 1 < size.x && markers(i + 1, j) == kKnown) || (j > 0 && markers(i, j - 1) == kKnown) || (j + 1 < size.y && markers(i, j + 1) == kKnown)))
            {
