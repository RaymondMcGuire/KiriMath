 ...
 * @Last Modified time: 2020-04-18 21:21:20
 */
#ifdef _MSC_VER
...
#include <kiri_math/factory.h>
#include <kiri_math/fbs_helpers.h>
...
bool ParticleSystemData3::lifeCycle() const
{
    return _lifeCycle;
}

void ParticleSystemData3::setLifeCycle(bool newLifeCycle)
{
    _lifeCycle = newLifeCycle;
}

double ParticleSystemData3::maxLifeCycle() const
{
    return _maxLifeCycle;
}

void ParticleSystemData3::setMaxLifeCycle(double newMaxLifeCycle)
{
    _maxLifeCycle = newMaxLifeCycle;
}
...
ConstArrayAccessor1<double> ParticleSystemData3::lifeTimes() const
{
    return scalarDataAt(_lifeTimeIdx);
}

ArrayAccessor1<double> ParticleSystemData3::lifeTimes()
{
    return scalarDataAt(_lifeTimeIdx);
}
    ...
    const ConstArrayAccessor1<double> &newLifeTimes)
{
    ...
    size_t totalNumberOfParticles = oldNumberOfParticles + newNumberOfParticles;

    resize(totalNumberOfParticles);
    ...
    parallelFor(kZeroSize, newNumberOfParticles,
                [&](size_t i) {
    ...
    if (newLifeTimes.size() > 0)
    {
        parallelFor(kZeroSize, newNumberOfParticles,
                    [&](size_t i) {
                        lt[i + oldNumberOfParticles] = newLifeTimes[i];
                    });
    }
}
...
void ParticleSystemData3::set(const ParticleSystemData3 &other)
{
    ...
    _maxLifeCycle = other._maxLifeCycle;
    ...
    _lifeTimeIdx = other._lifeTimeIdx;
    ...
    const
{
    ...
    for (const auto &vectorData : _vectorDataList)
    {
        ...
        {
            newVectorData.push_back(kiri_mathToFbs(v));
        }
    ...
    for (const auto &neighbors : _neighborLists)
    {
        ...
        flatbuffers::Offset<fbs::ParticleNeighborList3> fbsNeighborList = fbs::CreateParticleNeighborList3(
            *builder,
    ...
    const fbs::ParticleSystemData3 *fbsParticleSystemData)
{
    ...
    // Copy scalars
    _lifeCycle = fbsParticleSystemData->lifeCycle();
    _maxLifeCycle = fbsParticleSystemData->maxLifeCycle();
    ...
    _lifeTimeIdx = static_cast<size_t>(fbsParticleSystemData->lifeTimeIdx());
