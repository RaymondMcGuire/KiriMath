...
void ScalarGrid3::serialize(std::vector<uint8_t> *buffer) const
{
    ...
    auto fbsResolution = kiri_mathToFbs(resolution());
    auto fbsGridSpacing = kiri_mathToFbs(gridSpacing());
    auto fbsOrigin = kiri_mathToFbs(origin());
...
{
    KIRI_MATH_ASSERT(dataSize().x * dataSize().y * dataSize().z == data.size());
